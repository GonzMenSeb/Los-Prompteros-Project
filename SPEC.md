...
| Sole size — one variant left, so no size question was owed | `guardrails.py` | `check_sole_sizes` discloses it: a size nobody chose is not a size the customer picked, and a 3XL reached a live kit in silence |
| Line removed on request — "I already have shoes" | `loop.py` | `_drop` is deterministic and zero model calls, removes only lines the message names, and **never empties the kit** — that falls through to the model. Emits `guardrail.lines_dropped` |
| Premise corrected — the trip is not what we researched | `loop.py` | `corrects_premise` throws away profile, slots and research and rebuilds, capped at `MAX_REBUILDS`. Over the cap it emits `guardrail.premise_change_refused` and **says so** rather than ignoring the correction |
